function Worker1688() {
  ...
  var siteName = "datlaco.vn";
  var siteLink = "http://datlaco.vn";
  ...
  function iconAddProductForNewUI($button) {
    setTimeout(() => {
      ...
      //   ".img-list-wrapper .detail-gallery-turn-wrapper img.detail-gallery-img"
      // );
      const productImgs = $(".detail-gallery-preview img.preview-img");
      let productImage = "";
      if (productImgs.length > 0) {
        // productImage = productImgs[0].attributes[2].nodeValue;
        productImage = productImgs[0].src;
      } else {
        const elements = document.querySelectorAll(
          'img.detail-gallery-img[ind="1"]'
        );
        for (var i = 0; i < elements.length; i++) {
          productImage = elements[i].getAttribute("src");
          break;
        }
      }
      ...
      // tìm cặp sku 1
      const sku1Element = $(
        ".pc-sku-wrapper .sku-module-wrapper .sku-prop-module-name"
      );
      if (sku1Element.length > 0) {
        const skuName1 = sku1Element.length > 0 ? sku1Element[0].innerText : "";
        const skuValue1Element = $(
          ".pc-sku-wrapper .sku-module-wrapper .prop-item-wrapper .prop-item .prop-item-inner-wrapper.active .prop-name"
        );
        const skuValue1 =
